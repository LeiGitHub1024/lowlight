...
class Uformer(nn.Module):
    def __init__(self, img_size=128, in_chans=3,
        ...
        # Encoder
        self.encoderlayer_0 = ConvBlock(embed_dim, embed_dim, strides=1)
        self.skipconnection_0 = BasicUformerLayer(dim=embed_dim,
        ...
        self.encoderlayer_1 = ConvBlock(embed_dim*2, embed_dim*2, strides=1)


        self.skipconnection_1 = BasicUformerLayer(dim=embed_dim*2,
        ...
        self.encoderlayer_2 = ConvBlock(embed_dim*4, embed_dim*4, strides=1)
        self.skipconnection_2 = BasicUformerLayer(dim=embed_dim*4,
        ...
        self.encoderlayer_3 = ConvBlock(embed_dim*8, embed_dim*8, strides=1)
        self.skipconnection_3 = BasicUformerLayer(dim=embed_dim*8,
        ...
        self.buf_0 = ConvBlock_1(embed_dim*8, embed_dim*8, strides=1)
        
        self.resize_1 = upsample(embed_dim*8, embed_dim*4)
        self.buf_1= ConvBlock_1(embed_dim*4, embed_dim*4, strides=1)

        self.resize_2 = upsample(embed_dim*4, embed_dim*2)
        self.buf_2 = ConvBlock_1(embed_dim*2, embed_dim*2, strides=1)
        
        self.resize_3 = upsample(embed_dim*2, embed_dim)
        self.buf_3 = ConvBlock_1(embed_dim, embed_dim, strides=1)
    ...
    def forward(self, x, stage, mask=None):
        ...
        buf0 = self.buf_0(rsz0)
        
        rsz1 = self.resize_1(buf0)
        buf1 = self.buf_1(rsz1)
        
        rsz2 = self.resize_2(buf1)
        buf2 = self.buf_2(rsz2)

        rsz3 = self.resize_3(buf2)
        buf3 = self.buf_3(rsz3)

        rsz4 = self.resize_4(buf3)
        ...
        sc3 = self.skipconnection_3(conv3, mask=mask)
        deconv0 = torch.cat([up0,sc3],-1)
        deconv0 = self.decoderlayer_0(deconv0,mask=mask)
        
        up1 = self.upsample_1(deconv0)
        
        sc2 = self.skipconnection_2(conv2, mask=mask)
        deconv1 = torch.cat([up1,sc2],-1)
        deconv1 = self.decoderlayer_1(deconv1,mask=mask)

        up2 = self.upsample_2(deconv1)
        sc1 = self.skipconnection_1(conv1, mask=mask)
        deconv2 = torch.cat([up2,sc1],-1)
        deconv2 = self.decoderlayer_2(deconv2,mask=mask)

        up3 = self.upsample_3(deconv2)
        sc0 = self.skipconnection_0(conv0, mask=mask)
        deconv3 = torch.cat([up3,sc0],-1)
