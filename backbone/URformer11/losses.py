...
class DecomLoss(nn.Module):
    ...
    def forward(self,S_low,S_high,I_low,I_high,R_low,R_high):
        ...
        recon_loss_low = torch.mean(torch.abs(R_low * I_low - S_low))
        recon_loss_high = torch.mean(torch.abs(R_high * I_high - S_high))
        ...
        i_input_mutual_loss_low = 0

        loss = 1*recon_loss_high + 1*recon_loss_low + 0.01 * equal_R_loss + 0.2*i_mutual_loss + 0.15* i_input_mutual_loss_high + 0.15* i_input_mutual_loss_low
        return loss
