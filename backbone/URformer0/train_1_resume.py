...
from losses import CharbonnierLoss,MyLoss,SSIM,SSIMLoss,L1Loss
...
path_chk_rest = './model/model_normal350.pth'
utils.load_checkpoint(model_restoration, path_chk_rest)
start_epoch = utils.load_start_epoch(path_chk_rest) + 1
opt.nepoch = opt.nepoch + start_epoch
...
######### Loss ###########
criterion_0 = L1Loss().cuda()
criterion_1 = SSIMLoss().cuda()
...
for epoch in range(start_epoch, opt.nepoch + 1):
    ...
    for i, data in enumerate(train_loader, 0): 
        ...
        with torch.cuda.amp.autocast():
            ...
            loss0 = criterion_0(restored, target, epoch)
            loss1 = criterion_1(restored, mid, epoch)
            loss = loss0 + 0.2 * (1 - epoch/opt.nepoch)*loss1
