...
class ConvBlockBadder(nn.Module):
    def __init__(self, in_channel, out_channel, strides=1):
        super(ConvBlockBadder, self).__init__()
        self.strides = strides
        self.in_channel=in_channel
        self.out_channel=out_channel
        self.block = nn.Sequential(
...
class UNetBadder(nn.Module):
    def __init__(self, block=ConvBlockBadder,dim=32):
        super(UNetBadder, self).__init__()
        self.dim = dim
        self.ConvBlock1 = ConvBlock(3, dim, strides=1)
        self.pool1 = nn.Conv2d(dim,dim,kernel_size=4, stride=2, padding=1)
