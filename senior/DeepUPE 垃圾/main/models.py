  ...
  def _coefficients(cls, input_tensor, params, is_training):
    ...
    with tf.variable_scope('global'):
      ...
      current_layer = conv(current_layer, 8*cm*gd, 3, stride=2,
            ...
            scope="conv{}".format(1))
      current_layer = conv(current_layer, 8*cm*gd, 3, stride=1,
            batch_norm=params['batch_norm'], is_training=is_training,
            scope="conv{}".format(2))
      ...
      current_layer = conv(current_layer, 8*cm*gd, 3, stride=1,
            batch_norm=params['batch_norm'], is_training=is_training,
            scope="conv{}".format(10))
      current_layer = conv(current_layer, 8*cm*gd, 3, stride=1,
            batch_norm=params['batch_norm'], is_training=is_training,
            scope="conv{}".format(11))
      current_layer = conv(current_layer, 8*cm*gd, 3, stride=1,
            batch_norm=params['batch_norm'], is_training=is_training,
            scope="conv{}".format(12))
    ...
    with tf.variable_scope('local'):
      ...
      current_layer = conv(current_layer, 8*cm*gd, 3, activation_fn=None,
                           use_bias=False, scope='conv3')
      current_layer = conv(current_layer, 8*cm*gd, 3, activation_fn=None,
                           use_bias=False, scope='conv4')
      current_layer = conv(current_layer, 8*cm*gd, 3, activation_fn=None,
                           use_bias=False, scope='conv5')
      current_layer = conv(current_layer, 8*cm*gd, 3, activation_fn=None,
                           use_bias=False, scope='conv6')
