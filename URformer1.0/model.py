...
class Uformer(nn.Module):
    def __init__(self, img_size=128, in_chans=3,
        ...
        self.resize_0 = upsample(embed_dim*16, embed_dim*8)
        self.ubuf_0 = ConvBlock_1(embed_dim*8, embed_dim*8, strides=1)
        
        self.resize_1 = upsample(embed_dim*8, embed_dim*4)
        self.ubuf_1= ConvBlock_1(embed_dim*4, embed_dim*4, strides=1)

        self.resize_2 = upsample(embed_dim*4, embed_dim*2)
        self.ubuf_2 = ConvBlock_1(embed_dim*2, embed_dim*2, strides=1)
        
        self.resize_3 = upsample(embed_dim*2, embed_dim)
        self.ubuf_3 = ConvBlock_1(embed_dim, embed_dim, strides=1)
        
        self.resize_4 = OutputProj(in_channel=embed_dim, out_channel=in_chans+1, kernel_size=3, stride=1)


        self.recover_0 = InputProj(in_channel=in_chans+1, out_channel=embed_dim, kernel_size=3, stride=1, act_layer=nn.LeakyReLU)
        # Bottleneck
        ...
        self.dbuf_1 = ConvBlock_1(embed_dim*2, embed_dim, strides=1)
        self.recover_1 = dowsample(embed_dim, embed_dim*2)
        
        self.dbuf_2 = ConvBlock_1(embed_dim*4, embed_dim*2, strides=1)
        self.recover_2 = dowsample(embed_dim*2, embed_dim*4)
        
        self.dbuf_3 = ConvBlock_1(embed_dim*8, embed_dim*4, strides=1)
        self.recover_3 = dowsample(embed_dim*4, embed_dim*8)
        
        self.dbuf_4 = ConvBlock_1(embed_dim*16, embed_dim*8, strides=1)
        ...
        self.decoder_buf_0 = ConvBlock_1(embed_dim*16, embed_dim*16, strides=1)
        ...
        self.decoder_buf_1 = ConvBlock_1(embed_dim*8, embed_dim*8, strides=1)
        ...
        self.decoder_buf_2 = ConvBlock_1(embed_dim*4, embed_dim*4, strides=1)
        ...
        self.decoder_buf_3 = ConvBlock_1(embed_dim*2, embed_dim*2, strides=1)
        ...
        self.conv_end = ConvBlock_1(2*embed_dim, 2*embed_dim, strides=1)
    ...
    def forward(self, x, stage, mask=None):
        ...
        rsz0 = self.resize_0(pool3)
        ubuf0 = self.ubuf_0(rsz0)
        
        rsz1 = self.resize_1(ubuf0)
        ubuf1 = self.ubuf_1(rsz1)
        
        rsz2 = self.resize_2(ubuf1)
        ubuf2 = self.ubuf_2(rsz2)

        rsz3 = self.resize_3(ubuf2)
        ubuf3 = self.ubuf_3(rsz3)

        rsz4 = self.resize_4(ubuf3)
        R, I = rsz4[:,:3,:,:], rsz4[:,3:4,:,:]
        ...
        dbuf1 = torch.cat([conv4,ubuf3],-1)
        dbuf1 = self.dbuf_1(dbuf1)
        rcv1 = self.recover_1(dbuf1)
        
        dbuf2 = torch.cat([rcv1,ubuf2],-1)
        dbuf2 = self.dbuf_2(dbuf2)
        rcv2 = self.recover_2(dbuf2)
        
        dbuf3 = torch.cat([rcv2,ubuf1],-1)
        dbuf3 = self.dbuf_3(dbuf3)
        rcv3 = self.recover_3(dbuf3)

        
        dbuf4 = torch.cat([rcv3,ubuf0],-1)
        dbuf4 = self.dbuf_4(dbuf4)
        rcv4 = self.recover_4(dbuf4)
        
        #Decoder
        up0 = torch.cat([rcv4,pool3],-1)
        up0 = self.upsample_0(up0)
        deconv0 = torch.cat([up0,conv3],-1)
        deconv0 = self.decoder_buf_0(deconv0)
        deconv0 = self.decoderlayer_0(deconv0,mask=mask)
        
        up1 = self.upsample_1(deconv0)
        
        deconv1 = torch.cat([up1,conv2],-1)
        deconv1 = self.decoder_buf_1(deconv1)
        deconv1 = self.decoderlayer_1(deconv1,mask=mask)

        up2 = self.upsample_2(deconv1)
        deconv2 = torch.cat([up2,conv1],-1)
        deconv2 = self.decoder_buf_2(deconv2)
        deconv2 = self.decoderlayer_2(deconv2,mask=mask)

        up3 = self.upsample_3(deconv2)
        deconv3 = torch.cat([up3,conv0],-1)
        deconv3 = self.decoder_buf_3(deconv3)
        deconv3 = self.decoderlayer_3(deconv3,mask=mask)
        
        endconv = self.conv_end(deconv3)

        # Output Projection
        y = self.output_proj(endconv)
