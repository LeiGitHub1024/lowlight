...
from losses import CharbonnierLoss,MyLoss,SSIM,TVLoss,SSIMLoss,SmoothLoss,Decom_Loss,Mutual_I_Loss
...
######### Loss ###########
reconst_loss = CharbonnierLoss().cuda()
smooth_loss = SmoothLoss().cuda()
decom_loss = Decom_Loss().cuda()
mutual_i_loss = Mutual_I_Loss().cuda()
...
stage0_checkpoint = './log/Uformer32_1015_1_0/models/model_best.pth'
...
for epoch in range(start_epoch, opt.nepoch + 1):
    ...
    for i, data in enumerate(train_loader, 0): 
        ...
        with torch.cuda.amp.autocast():
            R_low, I_low, restored = model_restoration(S_low, 1)
            ...
            loss0 = loss_reconst_dec + loss_ivref + 0.1 * smooth_loss_low + 0.1 * smooth_loss_normal
            #print(restored.type(), S_normal.type())
            loss1 = criterion1(restored, S_normal, epoch)
            rest = opt.nepoch - epoch 
            if rest<opt.nepoch//3:
                loss = loss1
            else:
                loss =  (1 - epoch/opt.nepoch) *loss0 + loss1
        ...
        if (i+1)%eval_now==0 and i>0:
            with torch.no_grad():
                ...
                for ii, data_val in enumerate((val_loader), 0):
                    ...
                    filenames = data_val[2]
                    with torch.cuda.amp.autocast():
                        r,I, restored = model_restoration(input_, 1)
                    restored = torch.clamp(restored,0,1)  
