...
parser.add_argument('--input_dir', default='/home/mist/lowlight/datasets/mit500/test',type=str, help='Directory of validation images')
parser.add_argument('--result_dir', default='./log/Uformer32_1021_3_0/results_mit/',type=str, help='Directory for results')
parser.add_argument('--weights', default='./log/Uformer32_1021_3_0/models/model_best.pth',type=str, help='Path to weights')
...
with torch.no_grad():
    ...
    for ii, data_test in enumerate(tqdm(test_loader), 0):
        ...
        if stage == 1:
            R, I,rgb_restored = model_restoration(rgb_noisy, 1)
            ...
            R, I = model_restoration(rgb_noisy, 0)
        #rgb_restored = tmp #*0.5 + rgb_restored*0.5
        #print(R.size(), I.size())
            
            #I_3 = torch.cat([I, I, I], 1)
            R = torch.clamp(R,0,1).cpu().numpy().squeeze().transpose((1,2,0))
            I = torch.clamp(I,0,1).cpu().squeeze(dim=0).numpy().transpose((1,2,0))
            #I_3 = torch.clamp(I_3,0,1).cpu().numpy().squeeze().transpose((1,2,0))
            ...
            utils.save_img(os.path.join(args.result_dir,'R_'+filenames[0]), img_as_ubyte(R))
            utils.save_img(os.path.join(args.result_dir,'I_'+filenames[0]), img_as_ubyte(I))
