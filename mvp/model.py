...
class InputProj(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        x1 = self.proj(x).flatten(2).transpose(1, 2).contiguous()  # B H*W C
        x2 = self.proj(torch.pow(x, 0.5)).flatten(2).transpose(1, 2).contiguous()  # B H*W C
        x = torch.cat([x1,x2],-1)
...
class AutoEncoder(nn.Module):
    def __init__(self, dim=64):
        ...
        self.Encoder = nn.Sequential(
            ...
            nn.Conv2d(3, dim, 3, 1, 1),   # [, 64, 96, 96]
            nn.BatchNorm2d(64),
            nn.Conv2d(dim, dim, 3, 1, 1), # [, 64, 96, 96]
            nn.ReLU(),
            nn.MaxPool2d(2, 2),             # [, 64, 48, 48]
            nn.BatchNorm2d(dim),
            nn.Conv2d(dim, dim, 3, 1, 1),  # [, 64, 48, 48]
            nn.ReLU(),
            nn.BatchNorm2d(dim),
            nn.Conv2d(dim, dim*2, 3, 1, 1), # [, 128, 48, 48]
            nn.ReLU(),
            nn.BatchNorm2d(dim*2),
            nn.Conv2d(dim*2, dim*2, 3, 1, 1), # [, 128, 48, 48]
            nn.ReLU(),
            nn.BatchNorm2d(dim*2),
            nn.Conv2d(dim*2, dim*4, 3, 1, 1), # [, 256, 48, 48]
            nn.ReLU(),
        ...
        self.Decoder = nn.Sequential(
            nn.ConvTranspose2d(dim*4, dim*2, 3 ,1, 1),   # [, 128, 24, 24]
            nn.ReLU(),
            nn.BatchNorm2d(dim*2),
            nn.ConvTranspose2d(dim*2, dim*2, 3, 2, 1, 1),   # [, 128, 48, 48]
            nn.ReLU(),
            nn.BatchNorm2d(dim*2),
            nn.ConvTranspose2d(dim*2, 64, 3, 1, 1),    # [, 64, 48, 48]
            nn.ReLU(),
            nn.BatchNorm2d(dim),
            nn.ConvTranspose2d(dim, dim//2, 3, 1, 1),      # [, 32, 48, 48]
            nn.ReLU(),
            nn.BatchNorm2d(dim//2),
            nn.ConvTranspose2d(dim//2, dim//2, 3, 1, 1),      # [, 32, 48, 48]
            nn.ConvTranspose2d(dim//2, dim//4, 3, 2, 1, 1),  # [, 16, 96, 96]
            nn.ReLU(),
            nn.BatchNorm2d(dim//4),
            nn.ConvTranspose2d(dim//4, 3, 3, 1, 1),         # [, 3, 96, 96]
            nn.Sigmoid()
        )
